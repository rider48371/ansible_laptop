# -*- coding: utf-8 -*-
# external_monitors.py — Three screen configuration
#
# Screen layout (left to right):
#   Screen 0 (Laptop)      : Alacritty, Chrome, Chat          (3 groups)
#   Screen 1 (Ext Mon 1)   : Music, Video                     (2 groups)
#   Screen 2 (Ext Mon 2)   : Email, Code, 8, 9, 10            (5 groups)
...
def init_widgets_list(systray=True):
    """Full widget bar. Pass systray=False for external monitors (Qtile only allows one Systray)."""
    widgets_list = [
              widget.Sep(
                       linewidth = 0,
                       padding = 6,
...
def init_screens():
    return [
        # Screen 0 — Laptop (Systray lives here)
        Screen(top=bar.Bar(widgets=init_widgets_list(systray=True),  size=28, opacity=0.97)),
        # Screen 1 — External Monitor 1 (identical bar, spacer instead of Systray)
        Screen(top=bar.Bar(widgets=init_widgets_list(systray=False), size=28, opacity=0.97)),
        # Screen 2 — External Monitor 2 (identical bar, spacer instead of Systray)
        Screen(top=bar.Bar(widgets=init_widgets_list(systray=False), size=28, opacity=0.97)),
    ]
